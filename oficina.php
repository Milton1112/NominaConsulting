...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['BuscarOficina'])) {
    $criterio = $_POST['BuscarOficina'];
}
    ...
    <header class="bg-primary text-white py-3 shadow-sm">
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div class="text-center flex-grow-1">
                <h1 class="fs-3 mb-0 fw-bold">Lista de Oficinas</h1>
            </div>
        </div>
    </header>
                    ...
                    <tbody>
                        <?php if (sqlsrv_has_rows($stmt)) : ?>
                            <?php while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) : ?>
                                <tr>
                                    <td><?php echo $row['id_oficina']; ?></td>
                                    <td><?php echo $row['Oficina']; ?></td>
                                    <td><?php echo $row['Empresa']; ?></td>
                                    <td>
                                        <button class="btn btn-info btn-sm rounded-pill px-3 me-2" onclick="window.location.href='templates/oficina/editar_oficina.php?id=<?php echo $row['id_oficina']; ?>'">
                        ...
                        <?php else : ?>
                            <tr>
                                <td colspan="6" class="text-center text-muted py-3">No se encontró ningúna oficina</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
