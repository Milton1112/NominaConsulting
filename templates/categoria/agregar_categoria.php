...
<body>
    <header class="bg-primary text-white py-3 shadow-sm">
        <div class="container d-flex justify-content-between align-items-center">
            <a href="../../categoria.php" class="btn btn-outline-light d-flex align-items-center">
                <i class="bi bi-arrow-left-circle me-2"></i> Regresar
            </a>
            <div class="text-center flex-grow-1">
                <h1 class="fs-3 mb-0 fw-bold">Agregar Categoría</h1>
            </div>
        </div>
    </header>
            ...
            <div class="card-body">
                <form action="" method="POST" novalidate>
                    <div>
                        ...
                        <div class="invalid-feedback">Por favor, ingresa la categoría.</div>
                    </div>      
                    <button type="submit" style="margin-top:30px;" class="btn btn-primary w-100">Agregar Categoría</button>
                </form>
            </div>
        </div>
    </div>
...
function agregarCategoriaProducto($conn){
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        // Llamar al procedimiento almacenado para insertar categoría
        $sp_stmt = sqlsrv_query($conn, "{CALL sp_insertar_categoria(?)}", $sp_params);

        // Verificar si la ejecución fue exitosa
        if ($sp_stmt) {
            echo '<script>alert("Se ha agregado la categoría."); window.location.href = "../../categoria.php";</script>';
        } else {
