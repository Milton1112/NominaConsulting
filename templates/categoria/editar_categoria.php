...
if (isset($_GET['id'])) {
    ...
    if ($stmt && sqlsrv_has_rows($stmt)) {
        ...
        // Almacenar los datos de la categoría en variables
        $nombre = $row['nombre'];
    ...
    <header class="bg-primary text-white py-3 shadow-sm">
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div class="text-center flex-grow-1">
                <h1 class="fs-3 mb-0 fw-bold">Actualizar Categoría</h1>
            </div>
        </div>
    </header>
        ...
        <div class="card mx-auto rounded" style="max-width: 600px;">
            ...
            <div class="card-body">
                <form action="" method="POST" novalidate>
                    ...
                    <button type="submit" style="margin-top: 20px;" class="btn btn-primary w-100" onclick="validarFormulario(event)">Actualizar Categoría</button>
                </form>
            </div>
        </div>
    </div>
...
function actualizarInfoCategoria($conn){
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $sp_stmt = sqlsrv_query($conn, "{CALL sp_actualizar_categoria(?, ?)}", $sp_params);

        // Verificar si la ejecución fue exitosa
        if ($sp_stmt) {
            echo '<script>alert("Se ha actualizado la categoría."); window.location.href = "../../categoria.php";</script>';
        } else {
