...
// Verificar si se ha recibido un ID de bono por URL
$id_bono = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if ($id_bono === 0) {
    die("ID de bono inválido.");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Ejecutar el procedimiento almacenado para eliminar el bono
    $params = array(
        array($id_bono, SQLSRV_PARAM_IN),
        array($eliminar_todos, SQLSRV_PARAM_IN)
    );

    $stmt = sqlsrv_query($conn, "{CALL sp_eliminar_bono14(?, ?)}", $params);

    if ($stmt) {
        // Mensaje de éxito
        $result = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
        echo '<script>alert("' . $result['Resultado'] . '"); window.location.href = "../../bono14.php";</script>';
    } else {
...
<head>
    ...
    <title>Eliminar Bono 14</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <header class="bg-primary text-white py-3 shadow-sm">
        <div class="container d-flex justify-content-between align-items-center">
            <a href="../../bono14.php" class="btn btn-outline-light d-flex align-items-center">Regresar</a>
            <div class="text-center flex-grow-1"><h1>Eliminar Bono 14</h1></div>
        </div>
    </header>
    ...
    <div class="container mt-5 mb-5">
        <div class="card mx-auto rounded" style="max-width: 500px;">
            <div class="card-body">
                <h5 class="card-title text-center">¿Deseas eliminar este bono o todos los bonos del empleado?</h5>
