...
function generarBono14($conn) {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Ejecutar el procedimiento almacenado para generar el bono 14
        $sp_stmt = sqlsrv_query($conn, "{CALL generar_bono14(?, ?)}", $sp_params);

        if ($sp_stmt) {
            echo '<script>alert("Bono 14 generado exitosamente."); window.location.href = "../../bono14.php";</script>';
        } else {
...
<body>
    <header class="bg-primary text-white py-3 shadow-sm">
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div class="text-center flex-grow-1"><h1>Generar Bono 14</h1></div>
        </div>
    </header>

    <div class="container mt-5 mb-5">
        <div class="card mx-auto rounded" style="max-width: 600px;">
            <div class="card-header bg-primary text-white">Formulario de Generación de Bono 14</div>
            ...
            <div class="card-body">
                <form action="" method="POST" novalidate onsubmit="validarFormulario(event)">
                    ...
                    <button type="submit" class="btn btn-primary w-100 mt-3">Generar Bono 14</button>
                </form>
            </div>
        </div>
    </div>
